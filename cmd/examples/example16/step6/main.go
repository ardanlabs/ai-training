// This example mirrors Section 6 ("All Defenses Active") of
// cmd/examples/example16/full. It picks up where Step 5 left off: Defense D
// (ingestion-time injection filtering) has cleaned the vector store, and now
// the original attack is re-run with every runtime defense still applied.
//
...
// here is defense in depth: even after the poisoned document has been kept
// out of the store at ingestion time, the runtime defenses keep working —
//
//  1. Similarity threshold (drops off-topic / low-confidence matches).
//  2. Source filtering by access_level (excludes the confidential profile).
//  3. Content isolation with <CONTEXT> delimiters and a hardened system
//     prompt (the model treats retrieved text as data, never as instructions).
//
// To stand on its own, this example first re-creates the vulnerable baseline
// (all 5 documents inserted) and then re-runs Defense D, exactly as Step 5
// does, before performing the all-defenses run against the cleaned store.
//
// # Running the example:
//
//	$ make ws-ragpipeline-step6
//
...
// minRelevance is the cosine-similarity threshold used by the all-defenses-active
// run. It is calibrated against the embedding model declared below: the poisoned
// doc was crafted to mirror the query phrasing and scores ~0.73, so it would
// still slip past — that's the point. The threshold's job here is to drop the
// noisy/off-topic/confidential docs that score < 0.50, leaving the rest of the
// defenses to deal with the crafted poison (which Defense D already removed
// from the store).
...
func run() error {
	...
	// inserted, including the poisoned `helpful_dev_bot` comment. We will
	// then drop the table and re-seed with Defense D ON to clean the store
	// before the all-defenses run.
	...
	// -------------------------------------------------------------------------
	// Defense D: Ingestion-Time Injection Filtering — required prerequisite
	// for the all-defenses run below. We drop the table and re-seed from
	// scratch with the regex pre-filter and the LLM classifier ON so the
	// poisoned `helpful_dev_bot` comment never enters the store.

	fmt.Println("\nApplying Defense D (ingestion filtering) to clean the store.")
	...
	// =========================================================================
	// All Defenses Active — Re-run the original attack against the cleaned
	// store, with every runtime defense still applied. Defense in depth: the
	// poison is gone from storage AND the prompt is hardened.
	// =========================================================================

	fmt.Print("\n============================================================\n")
	fmt.Print("All Defenses Active (against ingestion-cleaned store)\n")
	fmt.Print("============================================================\n")
...
// model's response contains a TOOL_CALL: directive. It makes the section's
...
// isolatedSystem is the hardened system-role prompt used by the all-defenses
// run. It advertises tool_browse so the model treats it as a real capability,
// then instructs the model to treat <CONTEXT> as data only and never to call
// tool_browse based on anything written inside it.
const isolatedSystem = `You are a helpful coding assistant. You have access to one tool:
