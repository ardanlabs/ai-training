...
// Bill: Added this method to expose the init call.
func (model *model) InitOpenVINOEncoder(modelPath string, device string, cacheDir string) error {
	if n := model.ctx.InitOpenVINOEncoder(modelPath, device, cacheDir); n != 0 {
		return fmt.Errorf("unable to init openvino encoder")
	}
